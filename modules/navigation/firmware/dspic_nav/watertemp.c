...
void watertemp_cntdwn(u8 idx)
{
    ...
    if (x <= settings.watertemp_rom.WTx[i]) {
        y = linear_interpolate(x, settings.watertemp_rom.WTx[i-1], settings.watertemp_rom.WTx[i],
                settings.watertemp_rom.WTy[i-1], settings.watertemp_rom.WTy[i]);
        break;
    }
